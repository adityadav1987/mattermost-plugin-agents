...
The plugin exposes Prometheus metrics for monitoring and cost tracking:

### LLM Token Usage Metrics

- `agents_llm_input_tokens_total`: Total input tokens consumed by LLM requests (labeled by user_id, team_id, llm_name)
- `agents_llm_output_tokens_total`: Total output tokens generated by LLM requests (labeled by user_id, team_id, llm_name)

These metrics enable organizations to track LLM usage costs per user and team, monitor token consumption patterns, and analyze usage data through their existing Prometheus monitoring infrastructure.

**Note**: Token usage tracking is supported for OpenAI, Anthropic, Azure OpenAI, OpenAI-compatible, and Cohere providers. ASage provider does not support token usage tracking.

### Other Metrics

- `agents_llm_requests_total`: Total number of LLM API requests (labeled by llm_name)
- `agents_api_time_seconds`: API endpoint response times
- `agents_http_requests_total` / `agents_http_errors_total`: HTTP request metrics
